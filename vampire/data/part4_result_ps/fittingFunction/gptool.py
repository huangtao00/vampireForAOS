...
class gpt():
	...
	def fitting_f(sefl,x,func,ax):
		...
		b.append(Curie)
		#ax.set_xticks(b)
		ax.plot(x,y,linestyle='-',color='blue',linewidth=4,label=dislabel) #add some style to this figure
	...
	def add_all_curve(self,datas):
		...
		for cid in range(0,curve_number):
			...
			ax.plot(xarray,yarray, linestyle=style,color=color,label=legend,marker=marker,linewidth=1,markersize=18)

		#plot a line 

		ax.plot(range(200,800),[0 for i in range(200,800)],color="black",linestyle="-",lw=2)
		self.fitting_f([x for x in range(200,800,10)],fit_curieT,ax) #fitting_f(sefl,x,func,ax):
		...
		ax.set_xticks(range(200,850,50))
