...
zfont = matplotlib.font_manager.FontProperties(fname=fontfile,size=30)
...
class gpt():
	...
	def fitting_f(sefl,x,func,ax):
		...
		b=[a*10 for a in x if a!=730]
		b.append(Curie)
		#ax.set_xticks(b)
		dislabel=r"$\sigma(T)=(%i/T-1)^{%.2f}\times%d$" %(Curie,gamma,const)
		ax.plot(x,y,linestyle='-',color='red',linewidth=4,label=dislabel) #add some style to this figure
	...
	def add_all_curve(self,datas):
		...
		for cid in range(0,curve_number):
			...
			ax.plot(xarray,yarray, linestyle=style,color=color,label=legend,marker=marker,linewidth=1,markersize=12)

		#plot a line 

		ax.plot(range(0,800),[0 for i in range(0,800)],color="black",linestyle="-",lw=2)
		self.fitting_f([x for x in range(10,800,10)],fit_curieT,ax) #fitting_f(sefl,x,func,ax):		
		#add x,y label
		...
		ax.set_ylim((0, 3700))
		ax.set_yticks(range(0,3700,200))
...
if __name__=="__main__":
	...
	color=["#ffaa44",'g','red','#7755ff',"#77ff88",'g','b','y',"r",'g','b','y',"r",'g','b','y']
