...
class gpt():
	...
	def add_all_curve(self,datas):
		...
		for cid in range(0,curve_number):
			...
			#print xarray,yarray
			ax.plot(xarray,yarray, linestyle=style,color=color,label=legend,marker=marker,linewidth=1,markersize=30)
...
class Datas():
	...
	#"""
		...
		for line in fd:
			...
			xdata.append(xd)
			#print yd
			ydata.append(float(yd)/10**5)#/10**6
...
if __name__=="__main__":
	...
	label=["Diameter$(nm)$",r"$K_{u}(\times 10^5J/m^3)$"]
	dset.add_label(label)

	#add curve to the figure
	legend=["T=350","T=0K","14 nm","10nm","12nm","14nm"]
	color=["#0000ff",'#000000','#000000','#007777','black','cyan']
	fnlist=["size_ku.dat"]
	style=["","",'']
	dataxy=[[1,7]]
