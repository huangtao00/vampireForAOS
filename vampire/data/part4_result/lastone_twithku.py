...
class gpt():
	...
	def add_all_curve(self,datas):
		...
		for cid in range(0,curve_number):
			...
			#print xarray,yarray
			ax.plot(xarray,yarray, linestyle=style,color=color,label=legend,marker=marker,linewidth=1,markersize=15)
		...
		for xlabel_i in ax.get_xticklabels(): 
			xlabel_i.set_fontsize(40)
		for ylabel_i in ax.get_yticklabels(): 
			ylabel_i.set_fontsize(40)
...
class Datas():
	...
	#"""
		...
		for line in fd:
			...
			xdata.append(xd)
			#print yd
			ydata.append(abs(float(yd)) /10**5)#/10**6
...
if __name__=="__main__":
	...
	label=["Temperature$(K)$",r"$K_{u}(\times 10^5J/m^3)$"]
	dset.add_label(label)

	#add curve to the figure
	legend=["T=350","T=0K","14 nm","10nm","12nm","14nm"]
	color=["red",'#000000','#000000','#007777','black','cyan']
	fnlist=["output.10nm"]
	style=["--","",'']
	dataxy=[[2,9]]# T with ku
